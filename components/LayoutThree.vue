<template>
    <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
            <div v-for="photo in photos" :key="photo.name" :class="photo.className" class="relative flex items-center space-x-3 rounded-lg bg-white px-6 py-5 shadow-sm focus-within:ring-2 focus-within:ring-indigo-500 focus-within:ring-offset-2 hover:border-gray-400" @click="setLayoutCode(photo.code, photo.layoutName)">
            <div class="min-w-0 flex-1">
                ...
                @change="cropImg"
            />
            <button @click="fileOPen(photo.code, photo.layoutName)" type="button" class="w-full rounded-lg border-2 border-dashed border-gray-300 p-12 text-center hover:border-gray-400 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2">
