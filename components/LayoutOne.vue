<template>
    <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            ...
            <button v-else @click="fileOPen" type="button" class="relative block w-full rounded-lg border-2 border-dashed border-gray-300 p-12 text-center hover:border-gray-400 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2">
...
onMounted(async () => {
    const result = util.getPhoto('http://co-foundry-api.test/api/image')
    await $fetch('http://co-foundry-api.test/api/image', {
        method: 'GET'
    })
    .then((result) => {
